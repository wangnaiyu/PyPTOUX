...
| `pypto-top-level-design-documents` | `active` within design-intent / `local-mirror` | 保守降权：设计意图 active，当前实现事实必须回 `pypto` 校验；local mirror path: `/Users/wny/Documents/2 领域 Area/工作/EASY CANN/样例工程&文件/pypto_top_level_design_documents` | 已 clone；branch `main`；commit `7faac0b910e40989a6bbd381a80595b65ab29708`；后续 refresh 需记录 snapshot |
...
- `pypto-tools` 的本地路径、mirror、adapter 和 demo 使用策略已由 Batch C 建立；后续 refresh 需记录 snapshot。
- `pypto-top-level-design-documents` 的本地 mirror 已建立；后续 refresh 需记录 snapshot。
...
- Batch E：已将 Batch A-D 的稳定规则回写到正式项目规则、索引和 `final-report.md`。

后续如需 refresh 任一外部 mirror，按 source registry 的 clone / local mirror policy 单独确认权限、检查本地改动并记录新的 snapshot。
