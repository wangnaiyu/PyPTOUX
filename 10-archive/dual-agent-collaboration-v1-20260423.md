...
>
> 文档基于仓库现有的分层结构(`02-knowledge/` → `04-uxdesign/` → `05-prototypes/` → `.agents/skills/pto-design-system/`)、已有的 agent skills 和写作约定,但**所有约定都允许随项目演进迭代**。
...
|---|---|---|
| **规划与方案** | 工作台演进路线、原型整合策略、技术选型 | `04-uxdesign/`、`09-docs/` |
| **业务知识检索** | 查 `pypto` / `CANN` / sample engineering、把结论沉淀到知识库 | `02-knowledge/`、`pypto-knowledge-source` |
| **结构与路由** | 决定新内容放哪里、怎么命名、要不要更新索引 | `09-docs/`、`pyptoux-content-router` |
...
| **设计系统沉淀** | 提取通用 tokens、components、patterns,从原型回流到 `.agents/skills/pto-design-system/` | PTO skill bundle |
...
| **视觉与动效** | 配色、字体、间距、过渡、微交互 | 同上 + `.agents/skills/pto-design-system/` 引用 |
...
- **Claude 不动知识库与设计文档正文**——`02-knowledge/`、`03-insights/`、`04-uxdesign/`、`09-docs/` 下的 markdown 由 Codex 维护。Claude 可以读,但**不直接改正文**。Claude 发现问题应在 `notes/` 里写"待澄清",而不是越界改。
...
- **索引由谁更新**:Codex 创建新课题/共享框架时,负责更新 `09-docs/03-indexes/`;Claude 新增原型实验文件不算"新课题",不需要动索引,但**进入 `app/` 阶段时**通知 Codex 更新 `prototypes.md`。
...
├── .agents/skills/pto-design-system/      [Codex 主写规则,Claude 主写实现]
│   ├── tokens/           [Codex 主写] token CSS 与生成产物
│   ├── css/              [Claude 主写] 可复用 UI class 实现
│   ├── references/       [Codex 主写] 原则、映射、preview gate
│   └── patterns/         [Codex + Claude] pattern contract 与实现
├── 07-assets/            [Claude 输出 → Codex 归档命名]
├── 08-references/        [Codex 维护] 外部资料索引
├── 09-docs/              [Codex 主写] 规范、模板、索引
└── 10-archive/           [Codex 操作] 归档冻结内容
```
...
4. **你 review + 可能让 Codex 介入**
   ...
   - 涉及多课题共性的视觉模式,让 Codex 评估是否提升到 `.agents/skills/pto-design-system/`
...
- 要接入 `.agents/skills/pto-design-system/css/style.css` 的真实组件代码
...
- **`.agents/skills/pto-design-system/references/`** 的设计原则、语气、约束(已有 `principles.md` / `voice-and-tone.md` / `usage-rules.md`)
- **`.agents/skills/pto-design-system/tokens/`** 的 tokens(虽然现在还是空的,Codex 会逐步填)
- **`.agents/skills/pto-design-system/references/`** 的组件规范(同上,逐步填)
- **`02-knowledge/`** 的业务理解
...
>
> - 视觉:从 `.agents/skills/pto-design-system/references/` 和 `.agents/skills/pto-design-system/tokens/`(如已有内容)出发
...
设计系统入口已收敛到 `.agents/skills/pto-design-system/`，当前协作节奏:

1. **每个新原型完成后**,你和 Codex 一起复盘:这个原型用到的哪些视觉决策、组件、模式应该沉淀?
2. **Codex 写规范** → `tokens/`、`references/`、`patterns/*.json` 写 token、组件映射和 pattern contract
3. **Claude 写实现** → `css/style.css` 或 `patterns/<pattern-id>/` 实现可复用代码片段
...
- **`pyptoux-component-generator`**:基于 `.agents/skills/pto-design-system/references/` 的组件规范,生成符合规范的实现脚手架。Claude 用。
...
4. **Codex**:用 `pyptoux-content-router` 确认路径正确,如有新增课题更新 `09-docs/03-indexes/prototypes.md`。
5. **你切到 Claude Desktop**:打开新会话,选项目根目录,首条 prompt:
   > 读 `04-uxdesign/02-swimlane-profiler/prd.md`(以最新版本为准)和 `05-prototypes/02-swimlane-profiler/notes/sample-data.md`,在 `experiments/html/swimlane-sync-v1.html` 实现一个单文件 demo,展示 mix 子图核间同步的等待段。视觉风格:从 `.agents/skills/pto-design-system/references/principles.md` 出发,**不参考其他现有 demo 的样式**。数据严格用 sample-data 里的内容。
...
1. **你发现复用模式**,让 Codex 评估:
   > 这个算子卡片在 X、Y、Z 三个 demo 里都出现,是否应该提升到 `.agents/skills/pto-design-system/02-components/`?
2. **Codex** 决策并在 `.agents/skills/pto-design-system/references/operator-card.md` 写规范(props、状态、视觉规则)
3. **Claude** 在 `.agents/skills/pto-design-system/css/style.cssoperator-card.html`(或 jsx)写参考实现
4. **后续原型**直接复制 `ui/operator-card.html` 的 markup + style
5. **Codex** 同步更新 `09-docs/03-indexes/designsystem.md`
...
- Claude 不修改 `02-knowledge/`、`03-insights/`、`04-uxdesign/`、`09-docs/` 下的正文
...
**索引同步责任**:
- Codex 创建新课题/共享框架时,负责更新 `09-docs/03-indexes/`
...
3. `09-docs/01-conventions/content-routing.md` —— 内容路由规则
4. `09-docs/01-conventions/agent-writing-style.md` —— 写作风格
5. `.agents/skills/pto-design-system/references/` —— 设计原则(已有部分内容)
...
- `.agents/skills/pto-design-system/css/style.css` —— 可复用 UI 实现
- `07-assets/` —— 输出截图、图标
...
- `08-references/` —— 外部资料索引
- `09-docs/` —— 规范、模板、索引

### 不要碰
- `.agents/` —— skill 定义
- `01-inbox/`、`10-archive/` —— Codex 操作
...
视觉起点:
- 从 `.agents/skills/pto-design-system/references/principles.md`、`voice-and-tone.md`、`usage-rules.md` 出发
- `.agents/skills/pto-design-system/tokens/`(tokens)如已有内容则直接使用
...
4. 在 `notes/<demo-name>.md` 用 `09-docs/02-templates/prototype-notes.md` 模板写:
   - 原型目标
   - 当前版本说明
   - 已验证结论
   - 待解决问题
   - 下一步
5. **不要**自己更新 `09-docs/03-indexes/`——索引由 Codex 负责

## 写作风格

- 中文优先(正文、说明、注释)
- 英文保留:目录名、文件名、代码标识、稳定术语(`HTML`、`JSX`、`PRD`、`UX`)
- 详见 `09-docs/01-conventions/agent-writing-style.md`
...
| 想做组件库浏览页 | 在 `.agents/skills/pto-design-system/css/style.css` 下做总览 HTML | 任意 |
...
**症状**:新增了一个原型课题但 `09-docs/03-indexes/prototypes.md` 没更新。
...
**症状**:同一个组件在 5 个原型里写了 5 次,样式略有差异,从来没沉淀到 `.agents/skills/pto-design-system/`。

**对策**:
- 每完成 2-3 个原型,让 Codex 跑一次"重复模式扫描":
  > 对比 `05-prototypes/` 下所有 experiments,找出在 3 个及以上 demo 里出现的 UI 模式,评估是否提升到 `.agents/skills/pto-design-system/02-components/`。
