...
1. **校验输入**
   - 所选目录存在且有 `index.html`
   - `index.html` 至少包含一个 `<script type="module" src="...">` 入口（否则当前打包器不适用，建议失败 + 明确错误）
2. **解析 index.html**
   - 收集 `<link rel="stylesheet">` 列表（远程 `href` 保留原 tag、不内联，见下方"远程资源"段）
   ...
   - **远程资源**：含 `://` 的 `href` / `src` 保留原 tag、不下载也不报错。产物联网可用，**离线不再 share-safe**。需要硬 share-safe 时应另行清洗（见 backlog "远程资源策略"项）
3. **CSS 内联**
...
4. **JS 模块解析与内联**
   ...
   - 每个模块体包进 **普通 function IIFE**（非 module scope）；这意味着 **top-level `await` 和 `import.meta` 不再保留原语义** —— 含这些特性的原型打包后产物可能无法运行，但本工具当前不会校验或报错
   - **首版采用 IIFE + 模块顺序拼接**，理由：A5 PMU 原型未使用 top-level await / dynamic import / `import.meta`。后续如果有原型用到，需要升级到真正的 `<script type="module">` + inline blob 或 data: URI 方案
5. **拓扑校验**
   - 检测循环依赖 → 报错
...
- macOS only（`.command` 是 macOS 专属；CLI 模式 `python3 bundler.py --cli` 可在任意 *nix 跑）
- 依赖：CLI 仅依赖 Python 3 标准库；GUI 通过 `osascript`（macOS 系统自带）实现，**不依赖 tkinter / 任何第三方 GUI 库**
- 不支持 `export default`、`import X from ...`（命名空间默认导入）；当前 PyPTOUX 内的原型未使用
- 不支持 dynamic `import()`、`import.meta`（实现把每个模块包成普通 IIFE，`import.meta.url` 含义会变；遇到时不报错，产物可能不可运行）
- 不支持 top-level `await`（被包进普通 function 后会变成语法错误；遇到时不报错，产物可能不可运行）
- 不支持 `import './setup.js'`（side-effect import，无绑定）；当前实现不收集，可能在产物里残留 `import` 语句
- 不支持 classic `<script src="...">` 多脚本原型（例如 `05-prototypes/05-pypto-studio/experiments/v1/`）；只识别 `<script type="module" src="...">`
- 远程 `<script src="https://...">` / `<link href="https://...">`：**保留原 tag，不内联**。产物联网时仍可工作，但**不再算 share-safe**（违反 `CLAUDE.md §5`"不依赖运行时远程资源"）
- 不内联本地 `.json` / `.csv` / 图片资源（首版）—— 如原型用 `fetch('./data.json')`，打包后双击仍会失败
- 不内联 CSS 里 `url(...)` 引用的本地资源（字体 / 图片）
- 不限制 `import` 的相对路径范围：理论上 `'../../foo.js'` 可以解析到 prototype 目录之外，存在把相邻文件意外打进外发 HTML 的风险。share-safe 流程需另行约束（见 backlog）
- 路径含空格 / 中文：测过；但路径含特殊符号（`|`、`?`、`*`）未保证
- 不混淆、不压缩、不 minify —— 保留可读性以便事后排查

> 已知限制的吸收路径：见同目录 `notes/backlog.md`。
