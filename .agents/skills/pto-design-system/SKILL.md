...
description: Use this skill when building PyPTOUX frontend prototypes, PTO-style pages, graph/workbench UI, or when retrofitting an existing demo to the PTO design system. Reuse the provided tokens, classes, patterns, and preview before inventing new visuals.
---

# PTO Design System Skill

This project skill lets Codex, Claude, or another frontend agent generate and retrofit PyPTOUX prototype pages in the PTO visual style.

## How to use this skill

1. Read this file first when a task mentions PTO style, frontend prototype design, graph/workbench UI, visual retrofits, or `07-designsystem`.
2. Tell the agent which workflow applies:
   - **New page from product requirement** → see *Workflow A*.
   - **Retrofit an existing demo to PTO style** → see *Workflow B*.
3. Open `.agents/skills/pto-design-system/design-system-preview.html` in a browser when visual confirmation is needed.
...
For layout-heavy work, also inspect `patterns/` (graph nodes, swimlane tasks, memory architecture, AIV/AIC core, pass-IR nodes). Use `graphviz/` only as reference material for graph-heavy demos; it is not a required baseline.
...
- The project routing rules in `AGENTS.md` / `CLAUDE.md` for where prototype output should be written
