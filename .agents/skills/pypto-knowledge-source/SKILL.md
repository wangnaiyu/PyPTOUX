...
| `demo-brief` | demo 叙事和素材需求 | 可衔接 `pypto-demo-data-filling`；只有继续设计或实现 PTO 风格 UI / demo / prototype 时，才衔接 `pto-design-system`。 |
...
如需继续做 demo 真实素材，衔接 `pypto-demo-data-filling`；如需继续做 PTO 风格 UI / demo / prototype 或 design system 应用，才衔接 `pto-design-system`。生成报告、研究简报、知识库文档或阅读型文档时，不管输出为 `.md`、`.html`、`.docx`、`.pptx`、PDF 或其他格式，都使用执行 agent 原生效果；Codex 用 Codex 原生效果，Claude 用 Claude 原生效果，其他 agent 同理。不因提到 UX、demo narrative、graph 或 visual analysis 而触发 `pto-design-system`。
