...
4. 按 demo 面板绑定真实素材：
   ...
   - 计算图面板 -> `program.json`、`topo.json`
   - 泳道图面板 -> `merged_swimlane.json`
   - 编译过程面板 -> `Pass_*` 目录、`function.cpp`
   - 调试说明面板 -> `run.log`
5. 先保证“面板素材真实”，再补解释性标题、摘要和标注；不要先写文案、后补素材。
6. 如果真实素材缺失，再退一步使用“基于真实结构整理”的摘要内容，不要伪造并不存在的字段、节点、性能数值或运行阶段。
7. 在产出里保留来源意识：哪些内容来自真实文件，哪些是为展示做的裁剪或简化。
8. 如果这次整理出了新的可复用数据集理解、素材组合套路或 demo 填充经验，回填到 `02-knowledge/` 对应主题下。
