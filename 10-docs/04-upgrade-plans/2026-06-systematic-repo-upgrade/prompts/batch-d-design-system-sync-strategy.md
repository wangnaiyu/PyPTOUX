...
本次 Batch D 的目标是建立 `yinyucheng0601/pto-design-system` 与 PyPTOUX 当前 `.agents/skills/pto-design-system`、`07-designsystem/` 之间的上游同步和本仓库投影方式，明确 mirror、手动同步、变更报告、投影范围、skill 边界和 preview gate。
...
- `10-docs/04-upgrade-plans/2026-06-systematic-repo-upgrade/batch-d-design-system-sync-strategy-plan.md`
...
请先向用户确认以下事项，不要一上来 clone、mirror、复制上游设计系统、改 `07-designsystem/` 或重写 PTO skill：

1. `yinyucheng0601/pto-design-system` 是否允许 clone 到本地 mirror？如果允许，本地 mirror 放在哪里？
2. 希望采用手动触发脚本、人工 copy，还是维护上游镜像？
3. `07-designsystem/` 是否只承载稳定投影，不承载 agent-facing preview 和实验 pattern？
4. `.agents/skills/pto-design-system` 是否继续作为 agent 使用的主入口？
5. 哪些内容允许投影进 `07-designsystem/`：tokens、CSS、patterns、docs、preview、screenshots、changelog？
...
- `.agents/skills/pto-design-system` 与 `07-designsystem/` 的边界如何记录
- 哪些内容被投影或明确不投影
- 哪些问题留给 Batch E
- 下一步是否进入 Batch E
