...
稳定结论最终应回写到正式位置，例如 `AGENTS.md`、`.agents/skills/*`、`02-knowledge/00-shared/*`、`.agents/skills/pto-design-system/*` 和 `09-docs/03-indexes/*`。
...
如后续需要改变 owner 边界，先记录到 `decisions.md`，再视情况同步到 `09-docs/01-conventions/dual-agent-collaboration.md`。
...
2. 建立 `.agents/skills/pto-design-system` 从上游 `yinyucheng0601/pto-design-system` 手动更新的方式，并判断 `.agents/skills/pto-design-system/` 应吸收哪些稳定内容。
...
- `prompts/batch-e-rule-consolidation.md`：正式启动 Batch E 时使用的 prompt。
...
- Batch E 使用 `prompts/batch-e-rule-consolidation.md`。
...
- `active`：任务仍在推进，目录保留在 `09-docs/04-upgrade-plans/`。
- `complete`：所有稳定结论已经回写到正式位置，目录保留为近期审计记录。
- `archived`：任务完成且不再需要日常检索时，整体移动到 `10-archive/upgrade-plans/2026-06-systematic-repo-upgrade/`。
