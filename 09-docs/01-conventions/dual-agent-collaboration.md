...
- 路径、命名、索引、literal 保留等基础规则仍遵守 `AGENTS.md` 与 `09-docs/01-conventions/content-routing.md`。
...
| 组件实现 | 在 `.agents/skills/pto-design-system/css/style.css` 提供可复用 UI 实现 |
...
| `.agents/skills/pto-design-system/css/style.css` | Claude | 可复用 UI 实现 |
| `.agents/skills/pto-design-system/tokens/`、`.agents/skills/pto-design-system/references/`、`.agents/skills/pto-design-system/SKILL.md` | Codex | tokens 规范、组件映射、原则约束与 skill 规则 |
| `09-docs/` | Codex | 索引、模板、全局规范 |
...
- Claude 不直接改写 `02-knowledge/`、`03-insights/`、`04-uxdesign/`、`09-docs/` 下的正文。
...
- `prompts/` 文件命名以 `09-docs/01-conventions/content-routing.md` 的主题记录层为准；其内容应保存可再次执行并尽量复现当次正式输出效果的复合 prompt，而不是用户原始 prompt 逐字稿。
- 复合 prompt 应揉合：
  - 用户原始目标与多轮对话中追加的修正信息。
...
#### `02-knowledge/` / `03-insights/` / `04-uxdesign/` / `09-docs/`：Codex 单边主写
...
3. `.agents/skills/pto-design-system/references/` 下已存在的原则与 guidance
...
- `.agents/skills/pto-design-system/tokens/` 的 tokens 与 `.agents/skills/pto-design-system/references/` 的 guidance 属于稳定基线，默认可以继承。
...
- `.agents/skills/pto-design-system/css/style.css`
...
- Claude 不默认继承 `experiments/` 历史 demo 的视觉与布局；PTO design system 的 tokens 与 guidance 可默认继承。见 §8.1、§8.2、§8.3。
